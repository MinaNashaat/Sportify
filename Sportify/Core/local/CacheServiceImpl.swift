...
class CacheServiceImpl: CacheService {
    ...
    func insertLeague(league: FavouriteLeague) {
        ...
        newLeague.setValue(Int32(league.leagueId),forKey: "leagueId")

        newLeague.setValue(league.leagueTitle,forKey: "leagueTitle")

        newLeague.setValue(league.leagueImage,forKey: "leagueImage")

        save()
    }

    func fetchLeagues() -> [FavouriteLeague] {
        let request = NSFetchRequest<NSManagedObject>(entityName: "FavouriteLeagues")

        do {
            let result = try context.fetch(request)

            return result.map { object in
                FavouriteLeague(leagueId : Int(object.value(forKey: "leagueId") as? Int32 ?? 0), leagueTitle:object.value(forKey: "leagueTitle") as? String ?? "", leagueImage:object.value(forKey: "leagueImage") as? String ?? "")
            }
        } catch {
            print(error)
            return []
